...
#model = torch.load("/home/maths/dual/mt6190837/mt6190837_a3/output/saved_model_2.pkl",map_location = torch.device('cpu'))
#model.to(device)
...
optimizer = Adafactor(model.parameters(),weight_decay=0.01,lr = 2e-5,relative_step=False)
...
for epoch in tqdm(range(NUM_EPOCHS)):
    ...
    for step,batch in enumerate(tqdm(training_loader)):
        ...
        if step%5000 == 0:
            torch.save(model,os.path.join(path_to_save,"saved_model_"+str(epoch)+"_"+str(step)))
